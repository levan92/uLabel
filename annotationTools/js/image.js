...
function image(id) {
	...
	this.zoomState = 0; //zoom state of the image
	...
	this.Zoom = function(amt) {
		...
		if(amt=='fitted') {
			this.scrollBarFlag = 0;
			this.im_ratio = this.browser_im_ratio;
		} else {
			this.im_ratio = this.im_ratio * amt;
		}

		// if the scale factor is below the original scale, then do nothing (do not make the image too small)
		...
		//Updates zoom state
		if (amt > 1) 	this.zoomState ++;
		else if (amt <1) this.zoomState --;
		else if (amt == 'fitted') this.zoomState = 0;
		//remove scroll bar if back to ground state
		if (this.zoomState == 0) 
			document.getElementById('main_media').style.overflow = 'hidden';
		else
			document.getElementById('main_media').style.overflow = 'auto';
	...
	this.GetAvailWidth = function() {
		return $(window).width() - $("#main_media").offset().left -250 -53;
