...
function StartDrawEvent(event) {
	...
	// Start message to the console:
	console.log('uLabel: Starting draw event...');
...
/**Mouse move event, for bounding boxes*/
...
function DrawCanvasMouseDown(event) {
	...
	// Draw line segment:
	draw_anno.line_ids.push(DrawLineSegment(draw_anno.div_attach,draw_x[n-1],draw_y[n-1],draw_x[n],draw_y[n],'stroke="#ff0000" stroke-width="2"',scale));
...
function StopDrawEvent() {
	...
	console.log('uLabel: Stopped draw event.');
}
