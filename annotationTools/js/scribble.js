...
function Scribble_canvas(tag) {
  ...
  this.colorseg = Math.floor(Math.random()*14); //color for seg mask
  ...
  this.cleanscribbles = function (){
	console.log("cleaning scribbles");
    this.clickX = new Array();
    this.clickY = new Array();
    this.clickDrag = new Array();
    this.clickColor = new Array();
  ...
  this.DrawToQuery = function () {
    ...
    console.log("What is this object query");
  ...
  this.compute = function (imageData, scribbleData) {
    ...
    var params = grabCut(imageHeap.byteOffset, scribbleHeap.byteOffset, imageData.height, imageData.width, 12);
//12: light blue
    //11: green
    //10:dark blue
    //13: yellow
  ...
  // changes to foreground/background/rubber
...
function EditBubbleEditScribble(){
  ...
  console.log("editbubbleeditscribble objEnter:" + main_handler.objEnter);
...
function DrawSegmentation ( div_attach, link, width, height, modified, aux){
  ...
  this.drawn_obj.setAttributeNS(this.xlinkNS,"href",link);
...
function ClearMask (id){
  ...
  if(q) {
	  ...
	  console.log("mask, child removed.")
  }
}
