...
function annotation(anno_id) {
    ...
    this.DrawPolygon = function (im_ratio, xp, yp) {
        ...
        if(xp.length==1) {
            ...
            var attr = 'fill="none" stroke="' + ClassColor(obj_name) + '" stroke-width="2"';
            ...
            var attr = 'fill="none" stroke="' + ClassColor(obj_name) + '" stroke-width="2" stroke-dasharray="9,5"';
    ...
    this.DrawPolyLine = function (xp, yp) {
        ...
        for(var i = 0; i < xp.length-1; i++) {
            // Draw line segment:
            this.line_ids.push(DrawLineSegment(this.div_attach, xp[i],yp[i],xp[i+1],yp[i+1],'stroke="#ff0000" stroke-width="2"',im_ratio));
          ...
          this.point_id = DrawPoint(this.div_attach,xp[0],yp[0],'r="3" fill="#FF4500" stroke="#ffffff" stroke-width="1.5"',im_ratio);
          ...
          $('#'+this.point_id).attr('onmouseover',"$('#'+draw_anno.point_id).attr('r',6,'stroke-width',3);");
          $('#'+this.point_id).attr('onmouseout',"if(draw_anno) {$('#'+draw_anno.point_id).attr('r',3,'stroke-width',1.5);}");
    };
