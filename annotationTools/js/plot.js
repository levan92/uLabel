...
function LMplot(xml,imagename) {
	...
	for(var i = 0; i < N; i++) {
		...
		if(!parseInt(obj.children("deleted").text())) {
			...
			// Draw polygon:
			var attr = 'fill="none" stroke="' + ClassColor(name) + '" stroke-width="4"';
			var scale = 1;
...
function DrawLineSegment(element_id,x1,y1,x2,y2,attr,scale) {
	...
	// Draw line segment:
	$('#'+element_id).append('<line xmlns="http://www.w3.org/2000/svg" id="' +
			dom_id + '" x1="' + x1*scale + '" x2="' + x2*scale +
			'" y1="' + y1*scale + '" y2="' + y2*scale + '" ' + attr + ' />');
...
function ClassColor(name) {
	...
	switch (name){
	case "Sky":		   return"#0088FF"; break; //blue
	case "Road":	   return"#B7B6BA"; break; //light grey
	case "WhiteLine":  return"#FFFFFF"; break; //white
	case "YellowLine": return"#FFFB00"; break; //yellow
	case "Kerb":	   return"#000000"; break; //black
	case "Pavement":   return"#E86FE6"; break; //pink
	case "LowVeg":	   return"#00FF4C"; break; //bright green
	case "HighVeg":	   return"#147330"; break; //dark green
	case "TreeTrunk":  return"#4F351B"; break; //brown
	case "Building":   return"#630C0C"; break; //reddish dark brown
	case "Object":  return"#FF0000"; break;  //red
	case "Pole":	   return"#839CB5"; break; //slate blue metallic
	default:return "#BB00FF"; break; //purple
	}

//	Following is a colour randomniser code
...
//	console.log("in hash objectcolor, return = "+ objectColors[hash]);
