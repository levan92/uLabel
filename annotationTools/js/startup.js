...
function StartupLabelMe() {
	...
	//disables right click context menu on all html elements
	...
	if(IsNetscape() || (IsMicrosoft() && (bversion>=4.5)) || IsSafari() || IsChrome()) {
		...
		console.log('uLabel starting up ......');
...
function FinishStartup() {
	...
	//Set more global shortcut handlers 
	$(document).bind('keydown.1', function(){main_media.Zoom(1.15);});
	$(document).bind('keydown.2', function(){main_media.Zoom(1.0/1.15);});
	$(document).bind('keydown.3', function(){main_media.Zoom('fitted');});
	$(document).bind('keydown.s', function(){CheckProgress();});
	$(document).bind('keydown.h', function(){EndCheckProgress();});
	$(document).bind('keydown.z', function(){main_handler.EraseSegment();});

	// Write "finished" messages:
	console.log('uLabel: finished loading');
...
function InitializeAnnotationTools(tag_button, tag_canvas){
	...
	var html_str = '<div id= "polygonDiv" class="annotatemenu">Polygon<br></br>Tool \
		...
		<img src="Icons/new_undo.png"  width="28" height="38" /> \
		</button> ';
